...
in
{
  ...
  home = {
    ...
    packages = with pkgs; [
      ...
      (writeShellApplication {
        name = "home-manage";
        text = builtins.readFile ./tools/home-manage.sh;
      })

      (writeShellApplication {
        name = "medial";
        text = builtins.readFile ./tools/medial.sh;
      })

      (writeShellApplication {
        name = "discordify";
        text = builtins.readFile ./tools/discordify.sh;
      })
    ];
  ...
  launchd.agents.ssh-load-keychain =
    let
      loader = pkgs.writeShellScript "ssh-load-keychain" ''
        sleep 60
